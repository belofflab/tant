...
bonus_cd = CallbackData("show_bonus", "level", "worker", "month", "option")
...
free_cd = CallbackData("show_free", "level", "worker", "type", "option")


def make_service_cd(level, worker="Natali_numerologist", type="0", service="0"):
    return service_cd.new(level=level, worker=worker, type=type, service=service)


def make_bonus_cd(level, worker="Natali_numerologist", month="0", option="1"):
    return bonus_cd.new(level=level, worker=worker, month=month, option=option)


def make_relationships_cd(level, worker="Natali_numerologist", key="0"):
...
def make_free_cd(level, worker="0", type="0", option="0"):
    return free_cd.new(level=level, worker=worker, type=type, option=option)
...
async def menu_keyboard(worker: str) -> InlineKeyboardMarkup:
    ...
    buttons = [
        ...
        {
            "text": "💖 Консультации без оплаты",
            "callback_data": make_free_cd(level=CURRENT_LEVEL + 1, worker=worker),
        },
        {
            "text": "🕰 Мои услуги",
