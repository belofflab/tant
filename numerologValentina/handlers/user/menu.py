...
import os
import uuid
from data.config import VALENTINA, SERVER_URL, CHANNEL_ID, BASE_DIR
...
MENU_CAPTION = """
<b>Приветствую тебя мой друг!</b>

Меня зовут Валентина. Я практикующий нумеролог/психолог, а ещё мотиватор!
Своим полем через расстояния, я очищаю и заряжаю своей энергией.
Я могу раскрыть ваш внутренний потенциал и скорректировать ваш жизненный путь. 
Я владею новейшими методиками Ставов, направленных на расширение основных сфер твоей жизни, которые дают гарантированный результат 🔥
"""
    ...
    if isinstance(message, types.Message):
        ...
        await message.message.edit_media(
            media=types.InputMediaPhoto(
                media=types.InputFile(VALENTINA), caption=MENU_CAPTION
            ),
...
async def list_matrix(callback: types.CallbackQuery, worker, page, **kwargs):
    await callback.message.edit_media(
        media=types.InputMediaPhoto(
            media=types.InputFile(VALENTINA),
